...
public class SchulstufeCalcRuleTest {
	...
	private void assertBerechtigt(List<VerfuegungZeitabschnitt> result) {
		...
		Assert.assertFalse(verfuegungZeitabschnitt.getBgCalculationInputAsiv().getBemerkungenMap().isEmpty());
	...
	private void assertNichtBerechtigt(List<VerfuegungZeitabschnitt> result) {
		...
		Assert.assertTrue(result.get(0).getBgCalculationInputAsiv().getBemerkungenMap().containsKey(MsgKey.ERWERBSPENSUM_ANSPRUCH));
		Assert.assertTrue(result.get(0).getBgCalculationInputAsiv().getBemerkungenMap().containsKey(MsgKey.SCHULSTUFE_KINDERGARTEN_2_MSG));
	}
