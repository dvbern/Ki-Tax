 ...
 */
public class MutationsMergerSchwyzTest {
	...
	public void setUp() throws Exception {
		gesuchsperiode = TestDataUtil.createGesuchsperiodeXXYY(2024, 2025);
		gesuchsperiode.setMandant(getMandantSchwyz());
		gemeinde = TestDataUtil.createGemeindeParis();
		gemeinde.setMandant(getMandantSchwyz());
	}


	@ParameterizedTest
	@CsvSource({
		// EG BP,		EG BP bis, 	EG BP%,	M Eingang,	M BP von,	M BP bis, 	M BP %,	Excepted GueltigAb, Bemerkung
		"2024-08-01,	2025-07-31, 60,		2024-03-15,	2024-08-01,	2025-07-31,	50,		2024-08-01,			",//'Pensum vermindert, rechtzeitig eingereicht'",
		"2024-08-01,	2025-07-31, 60,		2024-08-15,	2024-08-01,	2025-07-31,	50,		2024-09-01,			",//'Pensum vermindert, zu spaet eingereicht -> ab Folgemonat'",
		"2024-08-01,	2025-07-31, 60,		2024-09-01,	2024-08-01,	2025-07-31,	50,		2024-10-01,			",//'Pensum vermindert, zu spaet eingereicht -> ab Eingangsdatum'",
		"2024-08-01,	2025-07-31, 60,		2024-03-15,	2024-08-01,	2025-07-31,	70,		2024-08-01,			",//'Pensum erhoeht, rechtzeitig eingereicht'",
		"2024-08-01,	2025-07-31, 60,		2024-08-15,	2024-08-01,	2025-07-31,	70,		2024-09-01,			",//'Pensum erhoeht, zu spaet eingereicht -> ab Folgemonat'",
		"2024-08-01,	2025-07-31, 60,		2024-09-01,	2024-08-01,	2025-07-31,	70,		2024-10-01,			",//'Pensum erhoeht, zu spaet eingereicht -> ab Eingangsdatum'",
	})
	void test_anpassung_beschaeftigungspensum(
		LocalDate erstgesuchPensumVon,
		LocalDate erstgesuchPensumBis,
		int erstgesuchPensum,
		LocalDate muationEingang,
		LocalDate mutationPensumVon,
		LocalDate mutationPensumBis,
		int mutationPensum,
		LocalDate expectedGueltigAb
	) {
		// Erstgesuch vorbereiten
		Betreuung erstgesuchBetreuung = prepareErstgesuch(false,
			erstgesuchPensumVon, erstgesuchPensumBis, erstgesuchPensum);
		// Mutation vorbereiten
		Betreuung mutierteBetreuung = prepareMutation(erstgesuchBetreuung, muationEingang, false,
			mutationPensumVon, mutationPensumBis, mutationPensum);
		...
		setAnsprechberechtigtesPensumAbDatum(verfuegungsZeitabschnitteMutiert, muationEingang, mutationPensum);
		...
		for (VerfuegungZeitabschnitt verfuegungZeitabschnitt : zeitabschnitte) {
			System.out.println(verfuegungZeitabschnitt.getGueltigkeit().getGueltigAb() + " " +verfuegungZeitabschnitt.getAnspruchberechtigtesPensum());
		}

		//ueberprüfen
		Assert.assertNotNull(zeitabschnitte);
		Assert.assertEquals(12, zeitabschnitte.size());
		checkAllBefore(zeitabschnitte, expectedGueltigAb, erstgesuchPensum +20);
		checkAllAfter(zeitabschnitte, expectedGueltigAb, mutationPensum);
	}

	@ParameterizedTest
	@CsvSource({
		// % GS1,	% GS2,	EG mit 2 GS,	M Eingang,	M FamsitAb, 			EG BP,		EG BP bis, 	EG BP%,	M Eingang,	M BP von,	M BP bis, 	M BP %,	Excepted GueltigAb, Bemerkung
		// EG BP,		EG BP bis, 	EG BP%,	M Eingang,	M BP von,	M BP bis, 	M BP %,	Excepted GueltigAb, Bemerkung
		"60,	70,	false,	2025-03-16,	2025-02-01,	2025-04-01,			",//'Pensum vermindert, rechtzeitig eingereicht'",
//		"2024-08-01,	2025-07-31, 60,		2024-08-15,	2024-08-01,	2025-07-31,	50,		2024-09-01,			",//'Pensum vermindert, zu spaet eingereicht -> ab Folgemonat'",
//		"2024-08-01,	2025-07-31, 60,		2024-09-01,	2024-08-01,	2025-07-31,	50,		2024-10-01,			",//'Pensum vermindert, zu spaet eingereicht -> ab Eingangsdatum'",
//		"2024-08-01,	2025-07-31, 60,		2024-03-15,	2024-08-01,	2025-07-31,	70,		2024-08-01,			",//'Pensum erhoeht, rechtzeitig eingereicht'",
//		"2024-08-01,	2025-07-31, 60,		2024-08-15,	2024-08-01,	2025-07-31,	70,		2024-09-01,			",//'Pensum erhoeht, zu spaet eingereicht -> ab Folgemonat'",
//		"2024-08-01,	2025-07-31, 60,		2024-09-01,	2024-08-01,	2025-07-31,	70,		2024-10-01,			",//'Pensum erhoeht, zu spaet eingereicht -> ab Eingangsdatum'",
	...
	void test_anpassung_familiensituation(
		int pensumGS1,
		int pensumGS2,
		boolean erstgesuchWith2GS,
		LocalDate muationEingang,
		LocalDate famSitAbDatum,
		LocalDate expectedGueltigAb
	) {
		LocalDate pensumVon = LocalDate.of(2024, 8, 1);
		LocalDate pensumBis = LocalDate.of(2025, 7, 31);
		// Erstgesuch vorbereiten
		Betreuung erstgesuchBetreuung = prepareErstgesuch(erstgesuchWith2GS, pensumVon, pensumBis, pensumGS1);
		if (erstgesuchWith2GS) {
			final Gesuch erstgesuch = erstgesuchBetreuung.extractGesuch();
			TestDataUtil.addSecondGesuchsteller(erstgesuch);
//			TestDataUtil.calculateFinanzDaten(erstgesuch, FinanzielleSituationRechnerFactory.getRechner(erstgesuch));
//			erstgesuch.getFinanzDatenDTO().setMassgebendesEinkBjVorAbzFamGr(BigDecimal.valueOf(50000));
//			gesuch.setFinSitTyp(FinanzielleSituationTyp.SCHWYZ);
			Assert.assertNotNull(erstgesuch.getGesuchsteller2());
			erstgesuch.getGesuchsteller2().addErwerbspensumContainer(TestDataUtil.createErwerbspensum(
				gesuchsperiode.getGueltigkeit().getGueltigAb(), gesuchsperiode.getGueltigkeit().getGueltigBis(), pensumGS2));

			erstgesuch.getGesuchsteller2().setFinanzielleSituationContainer(new FinanzielleSituationContainer());
			assertNotNull(erstgesuch.getGesuchsteller2().getFinanzielleSituationContainer());
			erstgesuch.getGesuchsteller2().getFinanzielleSituationContainer().setFinanzielleSituationJA(new FinanzielleSituation());
			erstgesuch.getGesuchsteller2().getFinanzielleSituationContainer().getFinanzielleSituationJA().setNettolohn(BigDecimal.valueOf(50000));
		}
		// Mutation vorbereiten
		Betreuung mutierteBetreuung = prepareMutation(erstgesuchBetreuung, muationEingang, false, pensumVon, pensumBis, pensumGS1);
		List<VerfuegungZeitabschnitt> zaBetrMutiert = EbeguRuleTestsHelper.calculate(mutierteBetreuung);
		final List<VerfuegungZeitabschnitt> verfuegungsZeitabschnitteMutiert =
			EbeguRuleTestsHelper.runSingleAbschlussRule(monatsRule, mutierteBetreuung, zaBetrMutiert);
		setAnsprechberechtigtesPensumAbDatum(verfuegungsZeitabschnitteMutiert, muationEingang, pensumGS1);
		if (!erstgesuchWith2GS) {
			final Gesuch mutation = mutierteBetreuung.extractGesuch();
			mutation.setFinSitAenderungGueltigAbDatum(famSitAbDatum);
//			erstgesuchBetreuung.extractGesuch().setFinSitTyp(FinanzielleSituationTyp.SCHWYZ);
			TestDataUtil.addSecondGesuchsteller(mutation);
			//			TestDataUtil.calculateFinanzDaten(erstgesuch, FinanzielleSituationRechnerFactory.getRechner(erstgesuch));
			//			erstgesuch.getFinanzDatenDTO().setMassgebendesEinkBjVorAbzFamGr(BigDecimal.valueOf(50000));
			//			gesuch.setFinSitTyp(FinanzielleSituationTyp.SCHWYZ);
			Assert.assertNotNull(mutation.getGesuchsteller2());
			mutation.getGesuchsteller2().addErwerbspensumContainer(TestDataUtil.createErwerbspensum(
				gesuchsperiode.getGueltigkeit().getGueltigAb(), gesuchsperiode.getGueltigkeit().getGueltigBis(), pensumGS2));

			mutation.getGesuchsteller2().setFinanzielleSituationContainer(new FinanzielleSituationContainer());
			assertNotNull(mutation.getGesuchsteller2().getFinanzielleSituationContainer());
			mutation.getGesuchsteller2().getFinanzielleSituationContainer().setFinanzielleSituationJA(new FinanzielleSituation());
			mutation.getGesuchsteller2().getFinanzielleSituationContainer().getFinanzielleSituationJA().setNettolohn(BigDecimal.valueOf(50000));
		}
		...
		//		final LocalDate gueltigAb = muationEingang.with(TemporalAdjusters.firstDayOfNextMonth());

		for (VerfuegungZeitabschnitt verfuegungZeitabschnitt : zeitabschnitte) {
			System.out.println(verfuegungZeitabschnitt.getGueltigkeit().getGueltigAb() + " " +verfuegungZeitabschnitt.getAnspruchberechtigtesPensum());
		}

		int pensumVorher = erstgesuchWith2GS ? pensumGS1 + pensumGS2 : pensumGS1;
		int pensumNachher = erstgesuchWith2GS ? pensumGS1 : pensumGS1 + pensumGS2;

		//ueberprüfen
		Assert.assertNotNull(zeitabschnitte);
		Assert.assertEquals(12, zeitabschnitte.size());
		checkAllBefore(zeitabschnitte, expectedGueltigAb, pensumVorher);
		checkAllAfter(zeitabschnitte, expectedGueltigAb, pensumNachher);
	}

//	@Test
//	public void test_Reduktion_Rechtzeitig_aenderungUndEingangsdatumGleich() {
//
////		final LocalDate mEingang = gesuchsperiode.getGueltigkeit().getGueltigAb().plusMonths(6);
////		final LocalDate aenderungsDatumPensum = gesuchsperiode.getGueltigkeit().getGueltigAb().plusMonths(6);
//
...
//		final LocalDate egBpVon = LocalDate.of(2024, Month.AUGUST, 1);
//		final LocalDate egBpBis = LocalDate.of(2025, Month.JULY, 31);
//		final int egBpPensum = 60;
//		Betreuung erstgesuchBetreuung = prepareErstgesuch(false, egBpVon, egBpBis, egBpPensum);//prepareData(MathUtil.DEFAULT.from(50000), AntragTyp.ERSTGESUCH);
...
//
//		final List<VerfuegungZeitabschnitt> zeitabschnitteEG = verfuegungErstgesuch.getZeitabschnitte();
//
//		// Mutiertes Gesuch vorbereiten
//		final LocalDate mEingang = LocalDate.of(2024, Month.MARCH, 15);
//		final LocalDate mBpVon = LocalDate.of(2024, Month.AUGUST, 1);
//		final LocalDate mBpBis = LocalDate.of(2025, Month.JULY, 31);
//		final int mBpPensum = 50;
//		Betreuung mutierteBetreuung = prepareMutation(mEingang, false, mBpVon, mBpBis, mBpPensum);//prepareData(MathUtil.DEFAULT.from(50000), AntragTyp.MUTATION);
...
//		mutierteBetreuung.extractGesuch().setEingangsdatum(mEingang);
//		List<VerfuegungZeitabschnitt> zaBetrMutiert = EbeguRuleTestsHelper.calculate(mutierteBetreuung);
//		final List<VerfuegungZeitabschnitt> verfuegungsZeitabschnitteMutiert =
//			EbeguRuleTestsHelper.runSingleAbschlussRule(monatsRule, mutierteBetreuung, zaBetrMutiert);
//		setAnsprechberechtigtesPensumAbDatum(verfuegungsZeitabschnitteMutiert, mEingang, mBpPensum);
//
//		// mergen
//		List<VerfuegungZeitabschnitt> zeitabschnitte = EbeguRuleTestsHelper.runSingleAbschlussRule(
//			mutationsMerger,
...
//			verfuegungsZeitabschnitteMutiert);
//
//		final LocalDate gueltigAb = mEingang.with(TemporalAdjusters.firstDayOfNextMonth());
//
//		//ueberprüfen
//		Assert.assertNotNull(zeitabschnitte);
//		Assert.assertEquals(12, zeitabschnitte.size());
//		checkAllBefore(zeitabschnitte, gueltigAb.minusDays(1), egBpPensum);
//		checkAllAfter(zeitabschnitte, gueltigAb, mBpPensum);
//
//	}
...
//	}
	...
	private Betreuung prepareData(
		...
		boolean zweiGS,
		@NonNull LocalDate bpVon,
		@NonNull LocalDate bpBis,
		int bpPensum
	) {
		Betreuung betreuung =
			EbeguRuleTestsHelper.createBetreuungWithPensum(bpVon, bpBis,
				BetreuungsangebotTyp.KITA, 100, new BigDecimal(2000));
		final Gesuch gesuch = betreuung.extractGesuch();
		gesuch.setTyp(antragTyp);
		gesuch.getFall().setMandant(TestDataUtil.getMandantSchwyz());
		gesuch.setEingangsdatum(eingangsdatum);
		gesuch.setGesuchsperiode(gesuchsperiode);
//		TestDataUtil.createDefaultAdressenForGS(gesuch, zweiGS);
		...
		kindJA.setGeburtsdatum(LocalDate.of(2022, 1,1));
		kindJA.setUnterhaltspflichtig(true);
		kindJA.setLebtKindAlternierend(true);
		kindJA.setEinschulungTyp(EinschulungTyp.VORSCHULALTER);
		kindJA.setKinderabzugErstesHalbjahr(Kinderabzug.GANZER_ABZUG);
		kindJA.setKinderabzugZweitesHalbjahr(Kinderabzug.GANZER_ABZUG);
		gesuch.setKindContainers(kindContainers);

		TestDataUtil.calculateFinanzDaten(gesuch, FinanzielleSituationRechnerFactory.getRechner(gesuch));
		gesuch.getFinanzDatenDTO().setMassgebendesEinkBjVorAbzFamGr(BigDecimal.valueOf(50000));
		gesuch.setFinSitTyp(FinanzielleSituationTyp.SCHWYZ);
		Assert.assertNotNull(gesuch.getGesuchsteller1());
		gesuch.getGesuchsteller1().addErwerbspensumContainer(TestDataUtil.createErwerbspensum(
			gesuchsperiode.getGueltigkeit().getGueltigAb(), gesuchsperiode.getGueltigkeit().getGueltigBis(), bpPensum));
		...
		gesuch.getGesuchsteller1().getFinanzielleSituationContainer().getFinanzielleSituationJA().setNettolohn(BigDecimal.valueOf(50000));
	...
	private Betreuung prepareErstgesuch(
		boolean zweiGS,
		@NonNull LocalDate bpVon,
		@NonNull LocalDate bpBis,
		int bpPensum
	) {
		final LocalDate eingangsdatumRechtzeitig = gesuchsperiode.getGueltigkeit().getGueltigAb().minusMonths(1);
		final Betreuung erstgesuchBetreuung = prepareData(AntragTyp.ERSTGESUCH, eingangsdatumRechtzeitig, zweiGS, bpVon, bpBis, bpPensum);
		...
		verfuegungErstgesuch.setZeitabschnitte(EbeguRuleTestsHelper.runSingleAbschlussRule(
			...
			zabetrErtgesuch));
		erstgesuchBetreuung.setVerfuegung(verfuegungErstgesuch);
		return erstgesuchBetreuung;
	}

	@Nonnull
	private Betreuung prepareMutation(
		...
		final Betreuung mutierteBetreuung = prepareData(AntragTyp.MUTATION, eingangsdatumMuation, zweiGS, bpVon, bpBis, bpPensum);
//		Betreuung mutierteBetreuung = prepareData(MathUtil.DEFAULT.from(50000), AntragTyp.MUTATION);
//		mutierteBetreuung.extractGesuch().setEingangsdatum(eingangsdatumMuation);
//		mutierteBetreuung.initVorgaengerVerfuegungen(null, null);

		mutierteBetreuung.setVorgaengerId(erstgesuchBetreuung.getId());
				mutierteBetreuung.initVorgaengerVerfuegungen(erstgesuchBetreuung.getVerfuegung(), null);
				mutierteBetreuung.extractGesuch().setEingangsdatum(eingangsdatumMuation);
