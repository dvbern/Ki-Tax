...
describe('EbeguDatePipe', () => {
    ...
    it('should transform string of format "####" into string of format "#\'###"', () => {
        ...
        expect(EbeguNumberPipe.formatToNumberString('-762487')).toBe(
            "-762'487"
        );
        expect(EbeguNumberPipe.formatToNumberString('-163.50')).toBe('-163.50');
        expect(EbeguNumberPipe.formatToNumberString('-47851.48')).toBe(
            "-47'851.48"
        );
    });
    it('should remove not nummeric chars from string', () => {
        ...
        expect(EbeguNumberPipe.sanitizeInputString('157-62', true)).toBe(
            '15762'
        );
        expect(EbeguNumberPipe.sanitizeInputString('-762487', true)).toBe(
            '-762487'
        );
        expect(EbeguNumberPipe.sanitizeInputString('a163.50', true)).toBe(
            '163.50'
        );
        expect(EbeguNumberPipe.sanitizeInputString('-47851.48', true)).toBe(
            '-47851.48'
        );
    });
    it('should remove not nummeric chars from string no floating point allowed', () => {
        expect(EbeguNumberPipe.sanitizeInputString("1'000", false)).toBe(
            '1000'
        );
        expect(EbeguNumberPipe.sanitizeInputString('16a', false)).toBe('16');
        expect(EbeguNumberPipe.sanitizeInputString('157-62', false)).toBe(
            '15762'
        );
        expect(EbeguNumberPipe.sanitizeInputString('-762487', false)).toBe(
            '-762487'
        );
        expect(EbeguNumberPipe.sanitizeInputString('a163.50', false)).toBe(
            '16350'
        );
        expect(EbeguNumberPipe.sanitizeInputString('-4785148', false)).toBe(
            '-4785148'
        );
    });
});
