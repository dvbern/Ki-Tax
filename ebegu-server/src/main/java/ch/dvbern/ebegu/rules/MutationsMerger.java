...
public final class MutationsMerger {
	...
	private static void handleAnpassungErweiterteBeduerfnisse(
		...
		if (zeitabschnitt.isBesondereBeduerfnisseBestaetigt()
			...
			zeitabschnitt.getBgCalculationInput().addBemerkung(RuleKey.ANSPRUCHSBERECHNUNGSREGELN_MUTATIONEN, MsgKey.ANSPRUCHSAENDERUNG_MSG, locale);
		}
	}
	...
	private static void handleAnpassungAnspruch(
		...
		if (anspruchberechtigtesPensum > anspruchAufVorgaengerVerfuegung) {
			...
			if (!isMeldungRechzeitig(zeitabschnitt, mutationsEingansdatum)) {
				...
				zeitabschnitt.getBgCalculationInput().addBemerkung(RuleKey.ANSPRUCHSBERECHNUNGSREGELN_MUTATIONEN, MsgKey.REDUCKTION_RUECKWIRKEND_MSG, locale);
			}
		}
	}
