...
public final class TestDataUtil {
	...
	public static Map<EinstellungKey, Einstellung> prepareParameterMap(Gesuchsperiode gesuchsperiode) {
		var map = new HashMap<EinstellungKey, Einstellung>();
		prepareEinstellung(
			PARAM_PAUSCHALABZUG_PRO_PERSON_FAMILIENGROESSE_3,
			PAUSCHALABZUG_PRO_PERSON_FAMILIENGROESSE_3_FUER_TESTS,
			gesuchsperiode,
		...
		prepareEinstellung(
			...
			gesuchsperiode,
			map);
		prepareEinstellung(PARAM_GRENZWERT_EINKOMMENSVERSCHLECHTERUNG, "20", gesuchsperiode, map);
		prepareEinstellung(PARAM_PENSUM_KITA_MIN, "0", gesuchsperiode, map);
		prepareEinstellung(PARAM_PENSUM_TAGESELTERN_MIN, "0", gesuchsperiode, map);
		prepareEinstellung(PARAM_PENSUM_TAGESSCHULE_MIN, "0", gesuchsperiode, map);
		prepareEinstellung(GEMEINDE_KONTINGENTIERUNG_ENABLED, "false", gesuchsperiode, map);
		prepareEinstellung(GEMEINDE_BG_BIS_UND_MIT_SCHULSTUFE, EinschulungTyp.VORSCHULALTER.name(), gesuchsperiode, map);
		prepareEinstellung(PARAM_MAX_TAGE_ABWESENHEIT, "30", gesuchsperiode, map);
		prepareEinstellung(MAX_VERGUENSTIGUNG_VORSCHULE_BABY_PRO_TG, "150", gesuchsperiode, map);
		prepareEinstellung(MAX_VERGUENSTIGUNG_VORSCHULE_KIND_PRO_TG, "100", gesuchsperiode, map);
		prepareEinstellung(MAX_VERGUENSTIGUNG_KINDERGARTEN_PRO_TG, "75", gesuchsperiode, map);
		prepareEinstellung(MAX_VERGUENSTIGUNG_VORSCHULE_BABY_PRO_STD, "11.90", gesuchsperiode, map);
		prepareEinstellung(MAX_VERGUENSTIGUNG_VORSCHULE_KIND_PRO_STD, "8.50", gesuchsperiode, map);
		prepareEinstellung(MAX_VERGUENSTIGUNG_KINDERGARTEN_PRO_STD, "8.50", gesuchsperiode, map);
		prepareEinstellung(MAX_VERGUENSTIGUNG_PRIMAR_PRO_STD, "8.50", gesuchsperiode, map);
		prepareEinstellung(MAX_MASSGEBENDES_EINKOMMEN, "160000", gesuchsperiode, map);
		prepareEinstellung(MIN_MASSGEBENDES_EINKOMMEN, "43000", gesuchsperiode, map);
		prepareEinstellung(OEFFNUNGSTAGE_KITA, "240", gesuchsperiode, map);
		prepareEinstellung(OEFFNUNGSTAGE_TFO, "240", gesuchsperiode, map);
		prepareEinstellung(OEFFNUNGSSTUNDEN_TFO, "11", gesuchsperiode, map);
		prepareEinstellung(ZUSCHLAG_BEHINDERUNG_PRO_TG, "50", gesuchsperiode, map);
		prepareEinstellung(ZUSCHLAG_BEHINDERUNG_PRO_STD, "4.25", gesuchsperiode, map);
		prepareEinstellung(MIN_VERGUENSTIGUNG_PRO_TG, "7", gesuchsperiode, map);
		prepareEinstellung(MIN_VERGUENSTIGUNG_PRO_STD, "0.70", gesuchsperiode, map);
		prepareEinstellung(MIN_ERWERBSPENSUM_NICHT_EINGESCHULT, "20", gesuchsperiode, map);
		prepareEinstellung(MIN_ERWERBSPENSUM_EINGESCHULT, "40", gesuchsperiode, map);
		prepareEinstellung(ERWERBSPENSUM_ZUSCHLAG, "20", gesuchsperiode, map);
		prepareEinstellung(FACHSTELLE_MIN_PENSUM_SOZIALE_INTEGRATION, "20", gesuchsperiode, map);
		prepareEinstellung(FACHSTELLE_MAX_PENSUM_SOZIALE_INTEGRATION, "60", gesuchsperiode, map);
		prepareEinstellung(FACHSTELLE_MIN_PENSUM_SPRACHLICHE_INTEGRATION, "40", gesuchsperiode, map);
		prepareEinstellung(FACHSTELLE_MAX_PENSUM_SPRACHLICHE_INTEGRATION, "40", gesuchsperiode, map);
		prepareEinstellung(GEMEINDE_FERIENINSEL_ANMELDUNGEN_DATUM_AB,
			Constants.DATE_FORMATTER.format(gesuchsperiode.getGueltigkeit().getGueltigAb()), gesuchsperiode, map);
		prepareEinstellung(GEMEINDE_TAGESSCHULE_ANMELDUNGEN_DATUM_AB,
			Constants.DATE_FORMATTER.format(gesuchsperiode.getGueltigkeit().getGueltigAb()), gesuchsperiode, map);
		prepareEinstellung(GEMEINDE_TAGESSCHULE_ERSTER_SCHULTAG,
			Constants.DATE_FORMATTER.format(gesuchsperiode.getGueltigkeit().getGueltigAb()), gesuchsperiode, map);
		prepareEinstellung(GEMEINDE_TAGESSCHULE_TAGIS_ENABLED, "false", gesuchsperiode, map);
		prepareEinstellung(GEMEINDE_TAGESSCHULE_ZUSAETZLICHE_ANGABEN_ZUR_ANMELDUNG, "false", gesuchsperiode, map);
		prepareEinstellung(MAX_TARIF_MIT_PAEDAGOGISCHER_BETREUUNG, "12.24", gesuchsperiode, map);
		prepareEinstellung(MAX_TARIF_OHNE_PAEDAGOGISCHER_BETREUUNG, "6.11", gesuchsperiode, map);
		prepareEinstellung(MIN_TARIF, "0.78", gesuchsperiode, map);
		prepareEinstellung(GEMEINDE_ZUSAETZLICHER_GUTSCHEIN_ENABLED, "false", gesuchsperiode, map);
		prepareEinstellung(GEMEINDE_ZUSAETZLICHER_GUTSCHEIN_BETRAG_KITA, "0.00", gesuchsperiode, map);
		prepareEinstellung(GEMEINDE_ZUSAETZLICHER_GUTSCHEIN_BETRAG_TFO, "0.00", gesuchsperiode, map);
		prepareEinstellung(GEMEINDE_ZUSAETZLICHER_GUTSCHEIN_BIS_UND_MIT_SCHULSTUFE_KITA, EinschulungTyp.VORSCHULALTER.name(), gesuchsperiode, map);
		prepareEinstellung(GEMEINDE_ZUSAETZLICHER_GUTSCHEIN_BIS_UND_MIT_SCHULSTUFE_TFO, EinschulungTyp.VORSCHULALTER.name(), gesuchsperiode, map);
		prepareEinstellung(GEMEINDE_ZUSAETZLICHER_BABYBEITRAG_ENABLED, "false", gesuchsperiode, map);
		prepareEinstellung(GEMEINDE_ZUSAETZLICHER_BABYBEITRAG_BETRAG_KITA, "0.00", gesuchsperiode, map);
		prepareEinstellung(GEMEINDE_ZUSAETZLICHER_BABYBEITRAG_BETRAG_TFO, "0.00", gesuchsperiode, map);
		prepareEinstellung(GEMEINDE_ZUSAETZLICHER_ANSPRUCH_FREIWILLIGENARBEIT_ENABLED, "false", gesuchsperiode, map);
		prepareEinstellung(GEMEINDE_ZUSAETZLICHER_ANSPRUCH_FREIWILLIGENARBEIT_MAXPROZENT, "0", gesuchsperiode, map);
		prepareEinstellung(GEMEINDE_MAHLZEITENVERGUENSTIGUNG_ENABLED, "true", gesuchsperiode, map);
		prepareEinstellung(GEMEINDE_MAHLZEITENVERGUENSTIGUNG_EINKOMMENSSTUFE_1_VERGUENSTIGUNG_MAHLZEIT, "6",
			gesuchsperiode, map);
		prepareEinstellung(GEMEINDE_MAHLZEITENVERGUENSTIGUNG_EINKOMMENSSTUFE_1_MAX_EINKOMMEN, "50000", gesuchsperiode,
			map);
		prepareEinstellung(GEMEINDE_MAHLZEITENVERGUENSTIGUNG_EINKOMMENSSTUFE_2_VERGUENSTIGUNG_MAHLZEIT, "3",
			gesuchsperiode, map);
		prepareEinstellung(GEMEINDE_MAHLZEITENVERGUENSTIGUNG_EINKOMMENSSTUFE_2_MAX_EINKOMMEN, "70000", gesuchsperiode,
			map);
		prepareEinstellung(GEMEINDE_MAHLZEITENVERGUENSTIGUNG_EINKOMMENSSTUFE_3_VERGUENSTIGUNG_MAHLZEIT, "0",
			gesuchsperiode, map);
		prepareEinstellung(GEMEINDE_MAHLZEITENVERGUENSTIGUNG_FUER_SOZIALHILFEBEZUEGER_ENABLED, "true", gesuchsperiode,
			map);
		prepareEinstellung(GEMEINDE_SCHNITTSTELLE_KITAX_ENABLED, "false", gesuchsperiode,
			map);
		prepareEinstellung(GEMEINDE_MIN_ERWERBSPENSUM_NICHT_EINGESCHULT, "20", gesuchsperiode, map);
		prepareEinstellung(GEMEINDE_MIN_ERWERBSPENSUM_EINGESCHULT, "40", gesuchsperiode, map);
		prepareEinstellung(GEMEINDE_MAHLZEITENVERGUENSTIGUNG_MINIMALER_ELTERNBEITRAG_MAHLZEIT, "2",
			gesuchsperiode, map);
		prepareEinstellung(GEMEINDE_PAUSCHALBETRAG_HOHE_EINKOMMENSKLASSEN_AKTIVIERT, "false", gesuchsperiode, map);
		prepareEinstellung(GEMEINDE_PAUSCHALBETRAG_HOHE_EINKOMMENSKLASSEN_BETRAG_KITA, "0", gesuchsperiode, map);
		prepareEinstellung(GEMEINDE_PAUSCHALBETRAG_HOHE_EINKOMMENSKLASSEN_BETRAG_TFO, "0", gesuchsperiode, map);
		prepareEinstellung(GEMEINDE_PAUSCHALBETRAG_HOHE_EINKOMMENSKLASSEN_BETRAG_TFO_AB_PRIMARSCHULE, "0", gesuchsperiode, map);
		prepareEinstellung(GEMEINDE_PAUSCHALBETRAG_HOHE_EINKOMMENSKLASSEN_MAX_MASSGEBENDEN_EINKOMMEN_FUER_BERECHNUNG, "160000", gesuchsperiode, map);
		prepareEinstellung(GEMEINDE_KEIN_GUTSCHEIN_FUER_SOZIALHILFE_EMPFAENGER, "false", gesuchsperiode, map);
		prepareEinstellung(LATS_LOHNNORMKOSTEN, "10.39",	gesuchsperiode, map);
		prepareEinstellung(LATS_LOHNNORMKOSTEN_LESS_THAN_50, "5.2",	gesuchsperiode, map);
		String stichtag = gesuchsperiode.getGueltigkeit().getGueltigAb().getYear() + "-09-15";
		prepareEinstellung(LATS_STICHTAG, stichtag, gesuchsperiode, map);
		prepareEinstellung(EINGEWOEHNUNG_TYP, EingewoehnungTyp.KEINE.toString(), gesuchsperiode, map);
		prepareEinstellung(FKJV_MAX_DIFFERENZ_BESCHAEFTIGUNGSPENSUM, "100", gesuchsperiode, map);
		prepareEinstellung(FKJV_SOZIALE_INTEGRATION_BIS_SCHULSTUFE, "VORSCHULALTER", gesuchsperiode, map);
		prepareEinstellung(SPRACHLICHE_INTEGRATION_BIS_SCHULSTUFE, "VORSCHULALTER", gesuchsperiode, map);
		prepareEinstellung(FKJV_PAUSCHALE_BEI_ANSPRUCH, "false", gesuchsperiode, map);
		prepareEinstellung(FKJV_EINKOMMENSVERSCHLECHTERUNG_BIS_CHF, "null", gesuchsperiode, map);
		prepareEinstellung(FKJV_PAUSCHALE_RUECKWIRKEND, "false", gesuchsperiode, map);
		prepareEinstellung(ANSPRUCH_MONATSWEISE, "false", gesuchsperiode, map);
		prepareEinstellung(SCHNITTSTELLE_STEUERN_AKTIV, "false", gesuchsperiode, map);
		prepareEinstellung(FERIENBETREUUNG_CHF_PAUSCHALBETRAG, "30",gesuchsperiode, map);
		prepareEinstellung(FERIENBETREUUNG_CHF_PAUSCHALBETRAG_SONDERSCHUELER, "60",gesuchsperiode, map);
		prepareEinstellung(FKJV_FAMILIENSITUATION_NEU, "false", gesuchsperiode, map);
		prepareEinstellung(MINIMALDAUER_KONKUBINAT, "5", gesuchsperiode, map);
		prepareEinstellung(FINANZIELLE_SITUATION_TYP, "BERN", gesuchsperiode, map);
		prepareEinstellung(KITAPLUS_ZUSCHLAG_AKTIVIERT, "false", gesuchsperiode, map);
		prepareEinstellung(GEMEINDESPEZIFISCHE_BG_KONFIGURATIONEN, "false", gesuchsperiode, map);
		prepareEinstellung(ABHAENGIGKEIT_ANSPRUCH_BESCHAEFTIGUNGPENSUM,
			AnspruchBeschaeftigungAbhaengigkeitTyp.ABHAENGING.name(), gesuchsperiode, map);
		prepareEinstellung(KINDERABZUG_TYP, "ASIV", gesuchsperiode, map);
		prepareEinstellung(FKJV_MAX_PENSUM_AUSSERORDENTLICHER_ANSPRUCH, "100", gesuchsperiode, map);
		prepareEinstellung(AUSSERORDENTLICHER_ANSPRUCH_RULE, "ASIV", gesuchsperiode, map);
		prepareEinstellung(KESB_PLATZIERUNG_DEAKTIVIEREN, "false", gesuchsperiode, map);
		prepareEinstellung(BESONDERE_BEDUERFNISSE_LUZERN, "false", gesuchsperiode, map);
		prepareEinstellung(GESCHWISTERNBONUS_AKTIVIERT, "false", gesuchsperiode, map);
		prepareEinstellung(DAUER_BABYTARIF, "12", gesuchsperiode, map);
		prepareEinstellung(FKJV_TEXTE, "false", gesuchsperiode, map);
		prepareEinstellung(DIPLOMATENSTATUS_DEAKTIVIERT, "false", gesuchsperiode, map);
		prepareEinstellung(ZEMIS_DISABLED, "false", gesuchsperiode, map);
		prepareEinstellung(SPRACHE_AMTSPRACHE_DISABLED, "false", gesuchsperiode, map);
		prepareEinstellung(FREIGABE_QUITTUNG_EINLESEN_REQUIRED, "true", gesuchsperiode, map);
		prepareEinstellung(UNBEZAHLTER_URLAUB_AKTIV, "true", gesuchsperiode, map);
		prepareEinstellung(FACHSTELLEN_TYP, "BERN", gesuchsperiode, map);
		prepareEinstellung(AUSWEIS_NACHWEIS_REQUIRED, "false", gesuchsperiode, map);
		prepareEinstellung(PENSUM_ANZEIGE_TYP, "ZEITEINHEIT_UND_PROZENT", gesuchsperiode, map);
		prepareEinstellung(VERFUEGUNG_EINGESCHRIEBEN_VERSENDEN_AKTIVIERT, "true", gesuchsperiode, map);
		prepareEinstellung(ABWESENHEIT_AKTIV, "true", gesuchsperiode, map);
		prepareEinstellung(BEGRUENDUNG_MUTATION_AKTIVIERT, "false", gesuchsperiode, map);
		prepareEinstellung(VERFUEGUNG_EXPORT_ENABLED, "false", gesuchsperiode, map);
		prepareEinstellung(ZAHLUNGSANGABEN_ANTRAGSTELLER_REQUIRED, "false", gesuchsperiode, map);
		prepareEinstellung(VERANLAGUNG_MIN_UNTERSCHIED_MASSGEBENDESEINK, "0", gesuchsperiode,map);
		prepareEinstellung(ANSPRUCH_AB_X_MONATEN, "0", gesuchsperiode,map);
		prepareEinstellung(KITA_STUNDEN_PRO_TAG, "10", gesuchsperiode, map);
