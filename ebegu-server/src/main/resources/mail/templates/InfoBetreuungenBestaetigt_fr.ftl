...
Subject: ${fall.getPaddedFallnummer()}, ${gesuchsperiode.getGesuchsperiodeString()}, <@base64Header>kiBon <#if configuration.isDevmode>Système de test</#if> – <#if isSozialdienst>La demande pour ${gesuchsteller.fullName}<#else>La demande</#if> peut être validée</@base64Header>
