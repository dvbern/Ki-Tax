...
class MutationsMergerSchwyzTest {
	...
	private MonatsRule monatsRule = new MonatsRule(false);

	private MutationsMerger mutationsMerger = new MutationsMerger(Locale.GERMAN, false, false);

	private final BigDecimal DEFAULT_MASGEBENDES_EINKOMMEN = MathUtil.DEFAULT.from(50000);
	...
	void test_hoereMassgegebeneseinkommens_steigtFolgeMonatNachMutation() {
		...
		final List<VerfuegungZeitabschnitt> zeitabschnitteAfterMonatsRule =
			EbeguRuleTestsHelper.runSingleAbschlussRule(monatsRule, mutierteBetreuung, zeitabschnitts);
	...
	private void checkAllBefore(
		...
		zeitabschnitte.stream().
			...
			forEach(za ->
				Assert.assertEquals("Falsches MassgebendesEinkommenVorAbzFamgr in Zeitabschnitt " + za,
					massgegebenesEinkommeVorAbzugFamgr, za.getMassgebendesEinkommenVorAbzFamgr())
			);
	}
