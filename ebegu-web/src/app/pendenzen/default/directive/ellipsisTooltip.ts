...
export default class EllipsisTooltip implements IDirective {
    ...
    public constructor() {
        this.link = (scope: IScope, element: IAugmentedJQuery, attrs: any) => {
            // Die Direktive wird vor dem ng-bind ausgefuehrt, deswegen muessen wir ein watch einfuegen, sodass der
            // Code erst ausgefuehrt wird, wenn das ng-bind vom Element fertig ist und der Text deshalb stimmt.
            scope.$watch(attrs.ngBind, (interpolatedValue: any) => {
                if (element[0].offsetWidth < element[0].scrollWidth) {
                    // hiere wird vergliechen ob der Text groesser als der offsetWidth ist, in dem Fall wissen wir dass
                    // ellipsis hinzugefuegt wurden und fuegen title hinzu
