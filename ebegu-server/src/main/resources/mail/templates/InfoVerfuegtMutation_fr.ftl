...
Subject: ${fall.getPaddedFallnummer()}, ${gesuchsperiode.getGesuchsperiodeString()}, <@base64Header>kiBon <#if configuration.isDevmode>Système de test</#if> – Le changement <#if isSozialdienst>pour ${gesuchsteller.fullName}</#if> a été enregistré</@base64Header>
